...
export const Route = createFileRoute("/gallery")({
  ...
  validateSearch: (search: Record<string, unknown>) => {
    const category =
      typeof search["category"] === "string" && search["category"].trim()
        ? search["category"].trim().toLowerCase().slice(0, 191)
        : undefined;

    const associatedTo =
      typeof search["associatedTo"] === "string" &&
      search["associatedTo"].trim()
        ? search["associatedTo"].trim().toLowerCase().slice(0, 191)
        : undefined;
  ...
  loaderDeps: ({ search }) => ({
    documentMode:
      ...
      search.associatedTo === "company-documents"
        ? ("company-documents" as const)
        : search.category === "general" &&
            search.associatedTo === "certificates"
...
function GalleryPage() {
  const { galleryItems, images, page } = Route.useLoaderData();
  ...
  const companyDocuments =
    specialDocuments && search.associatedTo === "company-documents";

  const [mediaFilter, setMediaFilter] =
    useState<(typeof mediaFilters)[number]>("All");

  const [subject, setSubject] = useState("All Destination Types");
  ...
  const categoryOptions = useMemo(() => {
    ...
    for (const item of galleryItems) {
      if (!item.cmsCategoryValue || !item.cmsCategory) {
        continue;
      }

      if (item.cmsCategoryValue === "general" && !specialDocuments) {
        continue;
      }

      options.set(item.cmsCategoryValue, item.cmsCategory);
    }
  ...
  const associatedOptions = useMemo(() => {
    ...
    for (const item of galleryItems) {
      if (item.cmsCategoryValue !== categoryFilter) {
        continue;
      }

      if (!item.associatedToSlug || !item.associatedToName) {
        continue;
      }

      options.set(item.associatedToSlug, item.associatedToName);
    }
  ...
  const destinationTypes = useMemo(() => {
    ...
    for (const item of galleryItems) {
       ...
       */
      if (item.associatedToKind !== "destination") {
        continue;
      }
  ...
  const items = useMemo(() => {
    ...
    return galleryItems.filter((item) => {
       ...
       * Destination Type
       */
      if (subject !== "All Destination Types") {
        if (
          item.associatedToKind !== "destination" ||
      ...
      if (
        companyDocuments &&
        documentType !== "All" &&
        item.generalSettingsType !== documentType
      ) {
        return false;
      }
       ...
       * CMS Category.
       */
      if (categoryFilter && item.cmsCategoryValue !== categoryFilter) {
        return false;
      }
       ...
       * CMS Associated To.
       */
      if (associatedFilter && item.associatedToSlug !== associatedFilter) {
        return false;
      }
  ...
  return (
    <>
      <PageHero
        compact
        image={page.heroImageUrl ?? images.destRara}
      ...
      <section className="container-lux py-20 lg:py-24">
          ...
          >
            {[
              ...
              ...Array.from(
                new Set(
                  galleryItems
                    .map((item) => item.generalSettingsType)
        ...
        <div className="mt-8 rounded-3xl border border-border bg-card p-5 shadow-sm lg:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-foreground">Search & Filters</h2>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px_280px]">
            ...
            <label className="grid gap-1.5">
              ...
              <select
                ...
                onChange={(event) => {
                   ...
                   * Destination media.
                   */
                  if (nextCategory !== "destination") {
                    setSubject("All Destination Types");
                  }
            ...
            <label className="grid gap-1.5">
              ...
              <select
                ...
                disabled={!categoryFilter || associatedOptions.length === 0}
                onChange={(event) => {
                  setAssociatedFilter(event.target.value);
        ...
        {items.length > 0 ? (
          <ul className="mt-12 grid auto-rows-[13rem] grid-flow-dense grid-cols-2 gap-4 lg:grid-cols-4">
            {items.map((item, index) => {
               ...
               */
              const displaySpan =
                galleryMosaicPattern[index % galleryMosaicPattern.length];
              ...
              return (
                <Reveal
                  ...
                  delay={index * 45}
                  className={cn(
                    displaySpan === "tall" && "row-span-2",

                    displaySpan === "wide" && "col-span-2",
                  )}
                >
                  <button
                    type="button"
                    ...
                    <span className="absolute inset-x-0 bottom-0 p-5">
                      <span className="block text-[0.65rem] font-bold uppercase tracking-[0.2em] text-gold">
                        {item.category !== "Uncategorised"
                          ? item.category
                          : (item.cmsCategory ?? item.category)}
                      </span>
          ...
          <div className="mt-12 rounded-3xl border border-border bg-card p-12 text-center">
            ...
            <h2 className="mt-4 text-2xl">Nothing found</h2>
...
function VideoPlayer({
  ...
  item: (typeof Route.types.loaderData)["galleryItems"][number];
}) {
...
function safeEmbedUrl(
  ...
  if (!provider || !value || !["youtube", "vimeo"].includes(provider)) {
    return null;
  }
